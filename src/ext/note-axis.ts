...
import Plottable, {Axis, AxisOrientation, Components} from "plottable";
import NoteScale from './note-scale'

class NoteAxis extends Axis<any> {
  /**
  ...
  /**
   * Constructs a Note Axis.
   *
   * @constructor
   * @param {NoteScale} scale
   * @param {AxisOrientation} [orientation="bottom"] Orientation of this Category Axis.
   */
  constructor(scale: NoteScale, orientation: AxisOrientation = "bottom") {
   ...
   * @returns {any}
   */
  public requestedSpace(offeredWidth: number, offeredHeight: number): any {
  ...
  protected _sizeFromOffer(availableWidth: number, availableHeight: number) {
    ...
    return (Plottable.Component.prototype as any)._sizeFromOffer.call(this, availableWidth, availableHeight);
  }
   ...
   *
   * @param {NoteScale} scale - The scale being downsampled. Defaults to this Axis' scale.
   * @return {DownsampleInfo} an object holding the resultant domain and new stepWidth.
   */
  public getDownsampleInfo(scale: NoteScale = <NoteScale> this._scale, domain = scale.invertRange()): IDownsampleInfo {
   ...
   * @param {Plottable.NoteScale} scale The scale this axis is representing.
  ...
  private _measureTickLabels(axisWidth: number, axisHeight: number) {
    const thisScale = <NoteScale> this._scale;
  ...
  public renderImmediately() {
    ...
    const catScale = <NoteScale> this._scale;
